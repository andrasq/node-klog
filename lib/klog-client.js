...
function KlogClient( logname, options ) {
    ...
    this.port = options.qrpcPort || options.port || 4245;
...
KlogClient.createClient = function createClient( logname, options, callback ) {
    ...
    klog.client = qrpc.connect(this.port, this.host, function(socket) {
        klog._socket = socket;
...
KlogClient.prototype._fflushKlog = function _fflushKlog( klog ) {
    ...
    klog.fflush(function(err) {
        if (err && !self._notifiedFlushError) {
            console.log("klogClient (%s) fflush error:", klog.journalName, err);
            self._notifiedFlushError = true;
...
//console.log("AR: read chunk head", nbytes, buf.slice(190, 200).toString(), buf.slice(nbytes - 10, nbytes).toString());
...
//console.log("AR: gathered tail", buf.slice(nbytes-10, nbytes).toString());
                    ...
                    if (qbuf.length <= chunkSize) return done();
