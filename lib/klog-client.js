...
KlogClient.prototype.fflush = function fflush( callback ) {
    if (this.journal) {
        ...
        var self = this;
        // avoid race conditions, fflush on next tick
        setImmediate(_doFflush, self);
...
//if (!err) console.log("AR: renamed file size", self.fs.readFileSync(grabbedJournalName).length);
...
console.log("AR: next chunked send");
