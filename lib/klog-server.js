...
KlogServer.prototype.createQrpcServer = function createQrpcServer( cb ) {
    ...
    server.addHandlerNoResponse('logname', function(req, res, next) {
//console.log("AR: logname =", req);
        logname = req.m;
    })

    server.addHandlerNoResponse('write', function(req, res, next) {
//console.log("AR: qrpc write to %s: req.m", logname, req.m);
nlines += 1;
        self.appendLog(logname, req.m);
    })

    server.addHandler('sync', function(req, res, next) {
//console.log("AR: qrpc sync", nlines);
nlines = 0;
        self.syncLog(logname, next);
    })
