...
KlogServer.prototype.close = function close( cb ) {
    ...
    this.closed = true;
    aflow.series([
        function(next) { self.httpServer ? self.httpServer.close(next) : next() },
...
// TODO: only create http and/or qrpc servers if port is specified, ie no default ports
    ...
    //if (klog.httpPort) klog.httpServer = klog.createExpressServer(whenListening);
...
KlogServer.prototype.createExpressServer = function createExpressServer( cb ) {
    ...
    if (0) app.use(function(req, res, next) {
        req.restiq = { _opts: { readBinary: true } };
        restiq.mw.readBody(req, res, next);
    })
    if (1) app.use(function(req, res, next) {
        ...
        req.on('end', function() {
            ...
            // TODO: would be faster to use string_decoder to gather as string,
    ...
    var server = app.listen(this.httpPort, function(err, ret) {
        cb(err, ret);
    })
...
KlogServer.prototype.createQrpcServer = function createQrpcServer( cb ) {
    ...
    server.addHandlerNoResponse('write', function(req, res, next) {
        // ...
    })
